...
<script>
  (function() {
    ...
    // 根据页面 lang 属性只切换文章正文和摘要的语言
    var htmlEl = document.documentElement;
    var currentLang = htmlEl.getAttribute('lang') || 'en';
    
    // 只处理文章内容区域的语言切换，不影响导航和页脚
    var articleEn = document.querySelector('.lang-en');
    var articleZh = document.querySelector('.lang-zh');
    
    if (articleEn && articleZh) {
      if (currentLang === 'zh') {
        articleEn.style.display = 'none';
        articleZh.style.display = '';
      } else {
        articleEn.style.display = '';
        articleZh.style.display = 'none';
      }
    }
  })();
</script>
